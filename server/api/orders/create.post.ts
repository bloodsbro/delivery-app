...
export default defineEventHandler(async (event) => {
  ...
  // SMS Log
  console.log(`Відправка SMS повідомлення на телефон ${created.delivery_contact_phone || created.customer.user.phone}: Ваше замовлення створено. ТТН: ${created.order_number}`);

  // Email
  if (created.customer.user?.email) {
     await sendEmail(created.customer.user.email, `Замовлення #${created.order_number} створено`, `Ваше замовлення успішно створено. Номер ТТН: ${created.order_number}.`);
  } else {
     console.log(`Email не вказано для замовлення #${created.order_number}, лист не відправлено.`);
  }
